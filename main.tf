...
k3d cluster create ${local.config.cluster_name} --servers 1 -p "80:80@loadbalancer" -p "443:443@loadbalancer" --api-port 6550 -p 30000-30004:30000-30004 --k3s-arg "--disable=traefik@server:*" --kubeconfig-update-default
sleep 2
...
resource "aws_route53_record" "sub_domain_wildcard" {
  ...
  name    = "*.apr.${data.aws_route53_zone.selected.name}"
  type    = "A"
  ttl     = "300"
